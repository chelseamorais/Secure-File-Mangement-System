...
def write(plaintext_filename, conn, socket,user,copy_command,current_dir):
    filename = filename_mapping(plaintext_filename)
    ...
    with FileLock(filename):
        if user in file_dictionaryW[filename]:
            with open(filename, "wb") as f:
                ...
                print("RECEIVED: (IN WRITE)", text)
                logging("WRITE", "File written with name: " + plaintext_filename, user,current_dir)
                f.write(text)
                #send_request_to_replicas(copy_command + ' ' + '"' + str(text) + '"')
                send_request_to_replicas(copy_command +'|'+ str(text))
        else:
            text = "Access denied"
...
def rename(plaintext_filename1, plaintext_filename2,current_dir,user):
    ...
    if user in file_dictionaryW[filename1]:
        logging("RENAME", "File with name " + plaintext_filename1 + " renamed to "+ plaintext_filename2, user,current_dir)
        ...
        response = "File renames successfully"
        with open(writeperm_encrypted, 'w') as convert_file:
            convert_file.write(json.dumps(file_dictionaryW))
    else:
        logging("RENAME", "Access Denied: File with name " + plaintext_filename1 + " not renamed to "+ plaintext_filename2, user,current_dir)
...
def cd(plaintext_filename, current_dir):
    ...
    if os.path.exists(filename):
        os.chdir(filename)
        logging("CD", "Current working Directory changed to: " + os.getcwd(), user, current_dir)
        rest1 = os.getcwd().split('umbc')[1]
        rest2 = ""
...
#file_dictionaryW=json.loads(data)
    ...
    while True:
        ...
        if command.split()[0] != "write":
            send_request_to_replicas(command)
        #send_request_to_replicas(command)
        ...
        if first_word_command == "write":
            ...
            #thread.start()
            text = write(filename1, conn, server_socket,user,copy_command,current_dir)
            # command = "write hello.txt"
        ...
        if first_word_command == "rename":
            ...
            #thread.start()
            data = rename(filename1, filename2,current_dir,user)
            send_response_to_client(data, conn)
